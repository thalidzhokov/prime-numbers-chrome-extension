...
В `manifest.json` по умолчанию заданы **английские** `description` и `action.default_title`. Ниже — варианты для листинга в консоли (в т.ч. русская локаль).

Скопируйте нужный блок в консоль разработчика при публикации расширения.

## Краткое описание (лимит 132 символа)
...
**Prime Numbers** — компактный попап для повседневных задач разработчика и не только.
...
Никакие данные не отправляются на серверы: всё выполняется только в окне расширения на вашем устройстве.
...
Nothing is sent to remote servers; everything runs only in the extension popup on your device.
